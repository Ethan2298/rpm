---
description: Commit, push to main, deploy the current MCP project to prod on Vercel - one command, never leave chat
---

# Push Current Project

Ship the current state of the MCP project to production. One command. Never leave chat.
...
Show Ethan:
- Files changed (focused on the current MCP integration layer in `mcp_servers/ghl/` and `api/mcp.py`)
...
- Report: deployed, endpoint live, and the current MCP project is serving the new version

### Step 6: Summary
Tell Ethan:
- What was shipped
- That it's live
- What the next MCP caller will experience differently (new tools, improved responses, etc.)
...
- Keep the wording project-aware: GHL is the current integration category, not the only thing the project can grow into
